...
4. **Application Repositoryは廃止せず、スコープアプリをテスト・Pre-Prod・本番へ配布する実行アーティファクト経路として残す。** GitHubは本番インスタンスへの直接デプロイ経路にしない。
...
| Pre-Prod | リリースリハーサル、メジャーアップグレード検証 | 本番同等性を守り、開発・恒常的な手修正を禁止すべき。 |
| 本番 | 本番 | AEMC/ReleaseOpsのコントローラー候補。公式ドキュメントでも通常は本番をコントローラーとする。 |

### 2.2 主な課題

- Dev→検証だけが更新セット、検証→Pre-Prod→本番がApplication Repositoryとなっており、**同じ変更の識別子と証跡が経路途中で分断**される。
...
5. **環境用途を固定する。** Pre-Prodと本番での直接開発を禁止し、検証・UAT中の無関係変更を制御する。
